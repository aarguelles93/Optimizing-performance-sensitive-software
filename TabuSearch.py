...
class TabuSearch:
    ...
    def optimize(self):
        ...
        for i in range(self.num_iterations):
            ...
            # Simple tabu check - avoid exact matches
            neighbors = [n for n in neighbors if n not in self.tabu_list]
            
            if not neighbors:
                # If all neighbors are tabu, create a random solution
                neighbors = [self.create_solution()]
            ...
            print(f"Iteration {i + 1}: Best Eval = {best_eval:.6f}")
